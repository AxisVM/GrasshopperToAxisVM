...
namespace GrassHopperToAxisVM
{
    ...
    {
        public List<Structs.AxisMember> drawPonts(List<AxisPoint> lista1)
        {
            List<Structs.AxisMember> returner = new List<Structs.AxisMember>();
            foreach (AxisPoint lista in lista1)
            {
                //if (lista.Value.drawing == false) { continue; }/// CHECK SENT
                ...
                //lista.Value.activeTriggered = false;/// SET CHCK FALSE
                //lista.Value.drawing = false;
                lista.Value.clearp();
                lista.Value.addp(out_id);
                returner.Add(lista.Value.Clone());

            }
            return returner;
        }
        public List<Structs.AxisMember> drawLines(List<AxisLine>lista123)
        {
            List<Structs.AxisMember> returner = new List<Structs.AxisMember>();
            foreach (AxisLine lista_c in lista123)
            {
                if (this.sendOnlyChanges && this.lines_sent.FindAll(x => x.GeometryEquals(lista_c.Value)).Count > 0)
                {
                    Structs.AxisLine lista_x = (Structs.AxisLine)this.lines_sent.Find(x => x.GeometryEquals(lista_c.Value));
                    RLineAttr[] lista1 = new RLineAttr[lista_x.axisid_line.Count()];
                    bool matAndCrs = false;
                    int material = -1; int crossi = -1;
                    if (lista_x.axisid_line.Count >= 1 && lista_c.Value.material != null && lista_c.Value.crosssection != null)
                    {
                        material = mats.AddFromCatalog((ENationalDesignCode)System.Enum.Parse(typeof(ENationalDesignCode), (lista_c.Value.material.designCode)), lista_c.Value.material.name);
                        crossi = crsec.AddFromCatalog((ECrossSectionShape)System.Enum.Parse(typeof(ECrossSectionShape), (lista_c.Value.crosssection.shape)), lista_c.Value.crosssection.name);
                        matAndCrs = true;
                    }
                    if (matAndCrs) //if material and crossection are also given
                    {
                        for (int k = 0; k < lista_x.axisid_line.Count(); k++)
                        {
                        ...
                        int xxxxxx = AxLines.BulkSetAttr(lista_x.axisid_line.ToArray(), lista1);
                    }
                    else //only geometric data
                    {
                        for (int k = 0; k < lista_x.axisid_line.Count(); k++)
                        {
                ...
                else //full SEND
                {
                    ...
                    lista_c.Value.clearp();
                    lista_c.Value.addp(op1); lista_c.Value.addp(op2);
                    int[] op11 = convert.toArray(op1);
                    int[] op22 = convert.toArray(op2);
                    ...
                    //lista_c.Value.activeTriggered = false;
                    //lista_c.Value.drawing = false;
                    lista_c.Value.clearl();
                    lista_c.Value.addl(ol);
                    returner.Add(lista_c.Value.Clone());
                }
            }
            return returner;
        }
        public List<Structs.AxisMember> drawMeshs(List<AxisMesh> lista)
        {
            List<Structs.AxisMember> returner = new List<Structs.AxisMember>();
            foreach (AxisMesh x in lista)
            {
                if (this.sendOnlyChanges && this.surfaces_sent.FindAll(xx => xx.GeometryEquals(x.Value)).Count > 0)
                {
                    Structs.AxisMesh lista_x = (Structs.AxisMesh)this.surfaces_sent.Find(xx => xx.GeometryEquals(x.Value));
                    ...
                    RSurface[] rsurf = lista_x.rSurfaceDataList;
                    for(int i = 0; i < rsurf.Length; i++)
                    {
                        rsurf[i].Attr = ATTR;
                    }
                    axisSurfaces.BulkSetSurfaces(lista_x.axisid_mesh.ToArray(),rsurf);
                }
                else // full send
                {
                    RSurfaceAttr ATTR = new RSurfaceAttr();
                    ...
                    //SET RSURFACE LST
                    x.Value.rSurfaceDataList = rline_listz;
                    //x.Value.activeTriggered = false;
                    //x.Value.drawing = false;
                    ////x.Value.deleting = false;
                    x.Value.clearm();
                    x.Value.addm(out_surface);

        ...
        public List<Structs.AxisMember> drawLines(List<AxisLine>lista123)
        {
            ...
            foreach (AxisLine lista_c in lista123)
            {
                if (this.sendOnlyChanges && this.lines_sent.FindAll(x => x.GeometryEquals(lista_c.Value)).Count > 0)
                {
                    ...
                    {
                        material = genMat((ENationalDesignCode)System.Enum.Parse(typeof(ENationalDesignCode), (lista_c.Value.material.designCode)), lista_c.Value.material.name);
                        crossi = genCrs((ECrossSectionShape)System.Enum.Parse(typeof(ECrossSectionShape), (lista_c.Value.crosssection.shape)), lista_c.Value.crosssection.name);
                        matAndCrs = true;
                        //crsec.ind
                    }

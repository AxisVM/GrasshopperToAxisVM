        ...
        public List<Structs.AxisMember> drawEdges(List<AxisMesh> lista)
        {
            ...
            foreach (AxisMesh x in lista)
            {
                if (this.sendOnlyChanges && this.edges_sent.FindAll(xx => xx.GeometryEquals(x.Value)).Count > 0)
                {
                    ...
                    {
                        int material = genMat((ENationalDesignCode)System.Enum.Parse(typeof(ENationalDesignCode), (x.Value.material.designCode)), x.Value.material.name);
                        int crossi = genCrs((ECrossSectionShape)System.Enum.Parse(typeof(ECrossSectionShape), (x.Value.crs.shape)), x.Value.crs.name);
